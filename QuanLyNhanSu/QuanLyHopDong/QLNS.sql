...
CREATE PROC pr_SelectHopDong
AS 
SELECT * FROM tbl_HOPDONG
CREATE PROC pr_Them_HopDong (  @sMaHopDong VARCHAR(10) , @dNgayKyHD DATETIME , @dThoiHan DATETIME , @sMaNV VARCHAR(10) , @sMaCV VARCHAR(10) , @sMaPB VARCHAR(10) , @fLuongCB FLOAT )
AS 
BEGIN
 if exists (select * from tbl_HOPDONG where PK_sMaHD = @sMaHopDong)
 BEGIN
 RAISERROR(N'Mã hợp đồng không được trùng lặp !!!', 16, 10)
;
 END
 else 
      BEGIN
	   INSERT INTO tbl_HOPDONG
       VALUES(@sMaHopDong , @dNgayKyHD , @dThoiHan , @sMaNV , @sMaCV , @sMaPB , @fLuongCB);
	   print N'Thêm thành công';
	   END
...
CREATE PROC pr_Sua_HopDong (  @sMaHopDong VARCHAR(10) , @dNgayKyHD DATETIME , @dThoiHan DATETIME , @sMaNV VARCHAR(10) , @sMaCV VARCHAR(10) , @sMaPB VARCHAR(10) , @fLuongCB FLOAT)

AS 
if not exists (select * from tbl_HOPDONG where PK_sMaHD= @sMaHopDong)
 BEGIN
 RAISERROR(N'Không tồn tại mã hợp đồng !!!', 16, 10);
 END
 else
 BEGIN
      
	  UPDATE tbl_HOPDONG SET  dNgaykyhd = @dNgayKyHD , dThoihan = @dThoiHan , FK_sMaNV = @sMaNV , FK_sMaCV = @sMaCV , FK_sMaPB = @sMaPB , fLuongcb = @fLuongCB
	  FROM tbl_CHUCVU
	  WHERE PK_sMaHD = @sMaHopDong;
	  
 END


  CREATE PROC pr_Xoa_HopDong (  @sMaHD NVARCHAR(10))

AS 
if exists (select PK_sMaHD from tbl_HOPDONG WHERE PK_sMaHD = @sMaHD)
BEGIN

           
           DELETE FROM tbl_HOPDONG  WHERE PK_sMaHD = @sMaHD ;   
END
